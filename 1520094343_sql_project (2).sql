...
SELECT DISTINCT Facilities.name AS facilities_name, CONCAT( firstname, ', ', surname ) AS member_name
...
SELECT * 
FROM (
SELECT Facilities.name AS facility_name, CONCAT( Members.surname,  ' ', Members.firstname ) AS member_name, 
...
SELECT Facilities.name, Facilities.facid, SUM(
...
GROUP BY Facilities.name, Facilities.facid
HAVING revenue >1000
ORDER BY revenue
